...
print "NN approximation completed and printed to file at: " + str(
				round(lastSolutionTime, 2)) + " seconds."
...
while improvement < 20:  #stop if 20 iterations pass with no improvement
	for i in range(0, numNodes):
		for k in range(i + 1, numNodes):
			...
			removedPathLength = Matrix[appxTour[(i - 1) % (
							numNodes)]][appxTour[i]] + Matrix[appxTour[k]][appxTour[(k + 1) %
																																																															(numNodes)]]
			addedPathLength = Matrix[appxTour[(
							i - 1) % numNodes]][appxTour[k]] + Matrix[appxTour[i]][appxTour[(
											k + 1) % (numNodes)]]
			if addedPathLength <= removedPathLength:  #only swap if new path would be at least as good as current
				...
				if newLength <= minLength:
					...
					if timeit.default_timer() - start_time < timeLimit:
						...
						sys.exit("Time limit reached. See " + outputFileName +
															" for best solution found at " +
															str(round(lastSolutionTime, 2)) + " seconds.")
	improvement += 1

elapsed = timeit.default_timer() - start_time
print "tsp-2-opt has successfully completed execution in " + str(
