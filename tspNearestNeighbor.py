...
def tourLength(tour, Matrix):
    ...
    sum += Matrix[tour[-1]][tour[0]]; #add edge that returns to start
    return sum;
...
def approximate(Matrix, outputFileName):
    numNodes = len(Matrix);
    if numNodes > 1000:     
        iterations = 10;        #for large graphs, just try first 10 in list
    else:
        iterations = numNodes;  #for small graphs, try starting from every node
    ...
    for start in range (0, iterations):
        ...
        current = start;
        tour.append(current);       #add start vertex to tour
